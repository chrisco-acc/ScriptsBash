...
# Fonction pour scaler le DeploymentConfig
scale_deployment_config() {
    local dc_name=$1
    local replicas=$2

    # Commande pour scaler le DeploymentConfig
    echo "Scaling $dc_name à $replicas réplicas..."
    oc scale dc/$dc_name --replicas=$replicas
}
...
# Scale les DeploymentConfigs sélectionnés
for index in "${selected_indices[@]}"; do
    ...
    scale_deployment_config "$dc_name" "$replicas"
done
